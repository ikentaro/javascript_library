...
const divRaw     =(arr2d, i, scale)=>   { for( let k=0; k<arr2d[0].length; k++ ){ arr2d[i][k]=div(arr2d[i][k], scale); } }
const scaleSubRaw=(arr2d, j, i, scale)=>{ for( let k=0; k<arr2d[0].length; k++ ){ arr2d[j][k]=sub(arr2d[j][k], mul(scale, arr2d[i][k])); } }
