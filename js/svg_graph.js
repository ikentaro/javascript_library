...
window.addEventListener('DOMContentLoaded', ()=>{
	...
	// else if( selected.value==='sphere-bessel' ){
	//     graph.drawFunc(x=> math.sphereBessel(0, x), 0, 10, -1, 1);
	//     const dphi=360/(MAX_LAGUERRE+1);
	//     graph.line(0).attr({ stroke: hslToRGB(0, 100, 50) });
	//     for( let i=1; i<=MAX_BESSEL; i++ ){
	// 	graph.drawFuncAdd(x=> math.sphereBessel(i, x));
	// 	graph.line(i).attr({ stroke: hslToRGB(dphi*i, 100, 50) });
	//     }
	//     graph.setYLabel(0.5);
	//     graph.setXLabel(2.5);
