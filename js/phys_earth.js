...
window.addEventListener('load', async ()=>{
    ...
    const earthR=1.469e11; // m
    const moonR=3.85e8; // m

    const earthV=2*Math.PI*earthR/(365*24*60*60); // m/s
    const moonV=2*Math.PI*moonR/(365*24*60*60/12); // m/s 地球に対する相対速度

    const posEarth=[ 50, 0, 0 ];
    const posMoon =[ 50+50*moonR/earthR, 0, 0 ];
    const velEarth=[ 0, 0, earthV ];
    const velMoon =[ 0, 0, earthV+moonV ];

    three.camera.perspective({ pos: [ 60, 50, 60 ], lookAt: [ 0, 0, 0 ], near: 0.01*moonR, far: 1.1*earthR });
    three.config.dump();
    const G=6.6743015e-11; // m^3kg^{-1}s^{-2}
    ...
    const earth=three.mesh(three.geom.sphere(0.0001, 10, 10), three.material.standard({ map: earthTexture }));
    const moon=three.mesh(three.geom.sphere(0.000001, 10, 10), three.material.standard({ color: 'yellow',  }));
	...
	three.config.camera.position.set(posEarth[0]+1.5*posMoon[0], posEarth[1]+1.5*posMoon[1], posEarth[2]+1.5*posMoon[2]);
	three.config.camera.lookAt(posEarth[0], posEarth[1], posEarth[2]);
    }
    function calc(){
	...
	earth.position.set(posEarth[0], posEarth[1], posEarth[2]);
	moon.position.set(posMoon[0], posMoon[0], posMoon[2]);
