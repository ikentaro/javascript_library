...
//	console.log('===========\ndt   :', dt, 'now :', now, 'time', time);	
	for( let i=0; i<3; i++ ){
		...
		force[0]+=G*(pos[j][0]-pos[i][0])/Math.pow(r, 3);
		force[1]+=G*(pos[j][1]-pos[i][1])/Math.pow(r, 3);
		force[2]+=G*(pos[j][2]-pos[i][2])/Math.pow(r, 3);
//		console.log(r);
	    }
//	    console.log('F=', force);
	    vel[i][0]+=dt*force[0];
...
//	console.log('pos[2] :', pos[2]);
    ...
    function setPos(){ for( let i=0; i<3; i++ ) balls[i].position.set(pos[i][0], pos[i][1], pos[i][2]); }
